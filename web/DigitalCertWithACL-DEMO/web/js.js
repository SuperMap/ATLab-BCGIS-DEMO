    ...
    $("#enroll_btn").click(function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                //存放多个下载的url
                let certFilePath = FileURL + "/msp/" + username;
                let privkeyFilePath = FileURL + "/msp/" + data.filename;
                let url_arr = [certFilePath, privkeyFilePath];
                $("#cert_download").html("证书文件<a href=\"" + certFilePath + "\">下载</a>");
                $("#privkey_download").html("密钥文件<a href=\"" + privkeyFilePath + "\">下载</a>");
                    ...
                    case "estate":
                        ...
                        reader_Image.onload = function (evt_Image) {
                            ...
                            $.ajax({
                                ...
                                url: RESTURL + '/channels/txchannel/chaincodes/aclcc/AddRecord',
                                data: JSON.stringify({
        ...
        reader_PrvkeyPEM.onload = function (evt_Prvkey) {
            ...
            reader_PubkeyPEM.onload = function (evt_Pubkey) {
                ...
                $.ajax({
                    ...
                    url: RESTURL + '/channels/txchennal/chaincodes/bcgiscc/PutRecord',
                    data: JSON.stringify({
                        'args': [argsHash, args, signature, fileString_PubkeyPEM],
                        'username': getCookie("username"),
                        'orgname': getCookie("orgname")
    ...
    function showDetail(txid) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $("#result_input").html(FormatOutputTableDetail(data, txid));
                var jsonData = JSON.parse(data);
...
function FormatOutputUsual(data, txid) {
    var jsonData = JSON.parse(data);
    // console.log(jsonData);
    ...
    for (var i = 0; i < jsonData.length; i++) {
        ...
        for (var key in jsonData[i]) {
            ...
            if (key == "Value" || key == "Record") {
                str += "<p><label><b>交易ID:</b></label>" + txid + "</p>";
...
function FormatOutputUsualWithUrl(data, txid) {
    var jsonData = JSON.parse(data);
    // console.log(jsonData);
...
function FormatOutputTableDetail(data, txid) {
    var jsonData = JSON.parse(data);
    // console.log(jsonData);
    ...
    if (!jsonData.hasOwnProperty("parentRecordID")) {
        ...
        str += "<tr><td><b>父交易ID： </b>" + jsonData["parentRecordID"] + "</td></tr>";
    }
    for (var key in jsonData) {
        ...
        if (keyName == "持证主体") {
            str += "<tr><td><b>持证主体：</b><span id=\"txCZZT\">" + jsonData[key] + "</span></td></tr>";
            continue;
        }
        if (keyName == "证照编号") {
            str += "<tr><td><b>证照编号：</b><span id=\"txZZBH\">" + jsonData[key] + "</span></td></tr>";
            continue;
        }
        if (keyName == "数据哈希") {
            str += "<tr><td><b>数据哈希：</b><span id=\"dataHash\">" + jsonData[key] + "</span></td></tr>";
            continue;
        }

        str += "<tr><td><b>" + keyName + "：</b>" + jsonData[key] + "</td></tr>";
        keyName = "null";
