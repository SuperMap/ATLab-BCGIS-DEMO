...
app.post('/users', async function (req, res) {
	...
	let response = await helper.getRegisteredUser(username, orgName, ouStr, true);
...
app.post('/channels/:channelName/chaincodes/:chaincodeName/AddRecord', async function (req, res) {
	...
	switch (storageType) {
		...
		case "ipfs":
			await fs.writeFileSync('../web/tmp/' + hash, imgdata, 'binary');
			await ipfsClient.add("../web/tmp/" + hash, (res) => {
				jsonArgs.hash = res;
				console.log("=======>" + res);
...
app.get('/GetFileFromIPFS', async function (req, res) {
	...
	res.json(filename);
	// ipfsClient.get(filename, "../web/tmp/" + filename, "");
});
